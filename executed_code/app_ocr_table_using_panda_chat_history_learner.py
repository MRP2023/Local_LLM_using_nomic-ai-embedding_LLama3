...
def add_knowledge_to_db(vectorstore, text):
    new_doc = Document(
        page_content=text,
        metadata={"source": "user_provided", "added_at": str(datetime.now())}
    )
...
def ask_question(vectorstore, question, chat_history):
    ...
    # Check if this is new knowledge being provided
    is_new_knowledge = any(
        msg["role"] == "user" and "here's some information:" in msg["content"].lower()
        for msg in chat_history[-2:]  # Check last 2 messages
    )
    if is_new_knowledge:
        # Extract the new knowledge from previous message
        new_info = chat_history[-1]["content"]
        ...
        vectorstore.add_documents([new_doc])
        return "Thank you! I've added this information to my knowledge base."
    ...
    contextualize_q_prompt = ChatPromptTemplate.from_messages([
        ...
        ("human", "{input}"),  # Consistent variable name
    ])
    history_aware_retriever = create_history_aware_retriever(
        llm, retriever, contextualize_q_prompt
    )

    # Step 2: Answer generation (using retrieved docs)
    qa_system_prompt = """
    Answer the question based only on the following context (retrieved by Nomic embeddings):
    {context}
    
    Question: {input}
    
    Be concise. If the context doesn't contain the answer, respond:
    "I don't have information about this. You can teach me by saying: 'Here's some information: [your knowledge]'.
    """
    ...
    result = qa_chain.invoke({
        "input": question,  # Pass as 'input' to match templates
...
if question:
    ...
    if "here's some information:" in question.lower():
        with st.spinner("🤖 Learning new information..."):
            vectorstore = load_vectorstore()
            response = add_knowledge_to_db(vectorstore, question)
        ...
        with st.chat_message("🐍"):
            with st.spinner("🤖 Python is thinking..."):
                vectorstore = load_vectorstore()
                result = ask_question(vectorstore, question, st.session_state.chat_history)
